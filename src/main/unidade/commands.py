...
@app.cli.command(
    'carregar-unidades',
    params=[opt_ativo, opt_cod_emp_princ],
    short_help=srt_help
)
def carregar_unidades(ativo, cod_emp_princ):
    ...
    if cod_emp_princ:
        EMPRESAS: list[EmpresaPrincipal] = [
            EmpresaPrincipal.query.get(cod_emp_princ)
        ]
    else:
        EMPRESAS: list[EmpresaPrincipal] = EmpresaPrincipal.query.all()

    click.echo(f'Carregando Unidades...')
    ...
    for ind, emp in enumerate(EMPRESAS):
        ...
        infos = Unidade.carregar_unidades(
            cod_emp_princ=emp.cod,
            ativo=ativo
        )
    ...
    click.echo(f'Done! Total -> Inseridas: {total_inseridos} | Atualizadas: {total_atualizados} | Erros {erros}')
