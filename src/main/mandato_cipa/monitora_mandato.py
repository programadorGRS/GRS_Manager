...
class MonitoraMandato(Mandato):
    ...
    def rotina_monitorar_mandatos(
        ...
        email_conf = self.__sub_rotina_email_configs(
            id_empresa=id_empresa, cod_unidade=cod_unidade
        )
        ...
        if erros:
            ...
            if not df_erros.empty:
                ...
                # indicador
                indicadores_inline["pendencias"] = self.__sub_rotina_indicadores_erros(
                    df=df_erros
                )
                # aba excel
                df_erros_final = self.__tratamento_final_erros(
                    id_empresa=id_empresa, df=df_erros
                )
                tabelas_excel.append(("Pendencias", df_erros_final))

        if vencimentos:
            df_vencimentos = self.gerar_df_vencimentos(
                df=df, dias_limite=dias_ate_vencimento
            )
            if not df_vencimentos.empty:
                # indicador
                indicadores_inline["vencimentos"] = (
                    len(df_vencimentos),
                    dias_ate_vencimento,
                )
                # aba excel
                df_vencimentos_final = self.__tratamento_final_vencimentos(
                    df=df_vencimentos, id_empresa=id_empresa
                )
    ...
    def __sub_rotina_email_configs(id_empresa: int, cod_unidade: str | None = None):
        ...
        if cod_unidade:
            ...
            conv_un: MandatoConfigUnidade = (
                database.session.query(MandatoConfigUnidade)  # type: ignore
                .filter(MandatoConfigUnidade.id_unidade == UNIDADE.id_unidade)
                .first()
            )
            conf["destinatarios_email"] = conv_un.emails
            conf["nome_arquivos"] = (
                secure_filename(UNIDADE.nome_unidade).replace(".", "_").upper()
            )
            conf["assunto_email"] = f"Monitoramento CIPA - {UNIDADE.nome_unidade}"
        else:
            EMPRESA: Empresa = Empresa.query.get(id_empresa)
            conf_em: MandatoConfigEmpresa = (
                database.session.query(MandatoConfigEmpresa)
                .filter(MandatoConfigEmpresa.id_empresa == EMPRESA.id_empresa)
                .first()
            )
            conf["destinatarios_email"] = conf_em.emails
            conf["nome_arquivos"] = (
                secure_filename(EMPRESA.razao_social).replace(".", "_").upper()
            )
            conf["assunto_email"] = f"Monitoramento CIPA - {EMPRESA.razao_social}"
    ...
    def __sub_rotina_dados_mandatos(
        ...
        df.drop_duplicates(
            subset=["cod_mandato", "cod_funcionario"], inplace=True, ignore_index=True
        )
    ...
    def __sub_rotina_indicadores_erros(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for keys, patt in conf_ind:
            ind_aux = self.get_indicadores(
                df=df, key_cols=keys, data_cols=self.__get_cols(df=df, pattern=patt)
            )
    ...
    def __sub_rotina_email_message(
        self,
        indicadores_inline: dict[str, tuple[Any, ...]],
        ...
        body_params = {
            ...
            "cid_assinatura": getattr(assinatura, "cid", None),  # type: ignore,
            "vencimentos": None,
        ...
        arquivo_path = self.gerar_excel(dfs=tabelas_excel, nome=email_configs.get("nome_arquivos"))  # type: ignore
        ...
        # make email Message
        msg = Message(
            subject=email_configs.get("assunto_email"),  # type: ignore
    ...
    def __get_nomes_unidades(id_empresa: int, df: pd.DataFrame):
        ...
        unidades = (
            database.session.query(Unidade.cod_unidade, Unidade.nome_unidade)  # type: ignore
            .filter(Unidade.id_empresa == id_empresa)
            .filter(Unidade.cod_unidade.in_(cod_unidades))
        )

        df_unidades = pd.read_sql(sql=unidades.statement, con=database.session.bind)  # type: ignore

        df = df.merge(right=df_unidades, how="left", on="cod_unidade")
        return df
    ...
    def gerar_df_erros(self, df: pd.DataFrame):
        ...
        df = df[
            ...
            | (df["erros_main_mandato"].notna())  # noqa
        ]

        df.sort_values(
            by="erros_main_funcionario",
            ascending=False,
            inplace=True,
            ignore_index=True,
        )
    ...
    def __membro_ativo(df: pd.DataFrame):
        ...
        condicoes = (
            (df["funcionario_renunciou"] == True),  # noqa
            (df["funcionario_eleito"] == True),  # noqa
        ...
        df["membro_ativo"] = np.select(
            condlist=condicoes, choicelist=valores, default=False
        )
    ...
    @staticmethod
    def __erros_funcao(
        df: pd.DataFrame, msg_erro: str = "Candidato sem Função definida"
    ):
    ...
    @staticmethod
    def __erros_situacao(
        df: pd.DataFrame, msg_erro: str = "Candidato sem definição de Titular/Suplente"
    ):
    ...
    def __erros_membros(df: pd.DataFrame, msg_erro: str = "Mandato sem eleitos"):
        ...
        df_membros = df_membros.loc[df_membros["membro_ativo"] == True]  # noqa
        ...
        df = df.merge(right=df_membros, how="left", on="cod_mandato")

        df["possui_membros"].fillna(value=False, inplace=True)

        df["erros_mandato_membros"] = np.where(
            df["possui_membros"] == False, msg_erro, None  # noqa # type: ignore
        )
    ...
    def __get_main_err_cols(self, df: pd.DataFrame):
        ...
        for cols, col_final in conf:
            df = self.__join_err_cols(
                df=df, err_cols=cols, key_cols=cols_chave, final_col_name=col_final
            )
    ...
    @staticmethod
    def __join_err_cols(
        df: pd.DataFrame, err_cols: list[str], key_cols: list[str], final_col_name: str
    ):
    ...
    def gerar_df_vencimentos(self, df: pd.DataFrame, dias_limite: int):
        ...
        df.sort_values(
            by="dias_vencimento", ascending=True, inplace=True, ignore_index=True
        )
    ...
    def __dias_vencimento_mandatos(self, df: pd.DataFrame):
        ...
        df["dias_vencimento"] = (df["data_fim_mandato"] - hoje).dt.days  # type: ignore

        df["status_venc_mandato"] = df["dias_vencimento"].apply(
            self.__gerar_status_venc
        )
