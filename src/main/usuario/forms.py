...
class FormCriarConta(FlaskForm):
    ...
    nome_usuario = StringField(
        "Nome do Usuário", validators=[DataRequired(), Length(3, 100)]
    )
    email = StringField("E-mail", validators=[DataRequired(), Email()])
    tel = TelField(
        "Telefone (Opcional)",
        validators=[Optional(), Length(0, 20)],
        render_kw={"placeholder": "(11) 3741-9977"},
    )
    cel = TelField(
        "Celular (Opcional)",
        validators=[Optional(), Length(0, 20)],
        render_kw={"placeholder": "(11) 93741-9977"},
    )
    senha = PasswordField("Senha", validators=[DataRequired(), Length(6, 50)])
    confirmacao_senha = PasswordField(
        "Confirmação da Senha", validators=[DataRequired(), EqualTo("senha")]
    )
...
class FormEditarPerfil(FlaskForm):
    # NOTE: definido na route
    id_usuario: int

    username = StringField("Username", validators=[DataRequired(), Length(3, 50)])
    nome_usuario = StringField(
        "Nome do Usuário", validators=[DataRequired(), Length(3, 50)]
    )
    email = StringField("E-mail", validators=[DataRequired(), Email()])
    tel = TelField(
        "Telefone (Opcional)",
    ...
    def validate_email(self, email):
        usuario = Usuario.query.get(self.id_usuario)
...
class FormConfigUsuario(FlaskForm):
    ...
    tipo_usuario = SelectField(
        "Tipo de Usuário", choices=[], validators=[DataRequired()]
    )
    botao_salvar = SubmitField("Salvar")
...
class FormAlterarChave(FlaskForm):
    chave = PasswordField(
        "Chave de Integração", validators=[DataRequired(), Length(6, 50)]
    )
    confirmacao_chave = PasswordField(
        "Confirmação da Chave", validators=[DataRequired(), EqualTo("chave")]
    )
    botao_salvar = SubmitField("Salvar")
