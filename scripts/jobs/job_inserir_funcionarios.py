if __name__ == '__main__':
    ...
    for empresa_principal in EMPRESAS_PRINCIPAIS:
        empresas: list[Empresa] = (
            database.session.query(Empresa.id_empresa)
            .filter(Empresa.cod_empresa_principal == empresa_principal.cod)
            .all()
        )
        empresas = [i.id_empresa for i in empresas]
        for i in range(TENTATIVAS):
            print(f'{empresa_principal.nome} - {ACAO} {NOME_TABELA} - tentativa: {i + 1}/{TENTATIVAS}', end=' - ')
            try:
                qtd_inseridos = Funcionario.inserir_funcionarios(cod_empresa_principal=empresa_principal.cod, lista_empresas=empresas)
