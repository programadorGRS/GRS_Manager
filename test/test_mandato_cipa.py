...
from src.main.mandato_cipa.commands.commands import (
    carregar_hist_mandatos,
    monitorar_mandatos,
    sync_configs,
)
...
def test_carregar_mandatos(app: Flask):
    with app.app_context():
        empresa = (
            database.session.query(Empresa)  # type: ignore
...
def test_monitorar_mandatos_command(runner: CliRunner):
    args_list = ["--modo", 2, "--id-empresa", 333, "--id-unidade", 5311]
...
def test_erros_funcao():
    ...
    df2 = mon._MonitoraMandato__erros_funcao(df=df1)  # type: ignore

    msg_erro = "Candidato sem Função definida"
    assert_series = pd.Series(
        [msg_erro, pd.NA, msg_erro, pd.NA], name="erros_funcionario_funcao"
    )
...
def test_erros_situacao():
    ...
    df2 = mon._MonitoraMandato__erros_situacao(df=df1)  # type: ignore

    msg_erro = "Candidato sem definição de Titular/Suplente"
    assert_series = pd.Series(
        [msg_erro, pd.NA, msg_erro, pd.NA], name="erros_funcionario_situacao"
    )
...
def test_erros_eleicao():
    ...
    df2 = mon._MonitoraMandato__erros_eleicao(df=df1)  # type: ignore

    msg_erro = "Eleição em sem Data Final"
    assert_series = pd.Series(
        [msg_erro, pd.NA, msg_erro, pd.NA], name="erros_mandato_eleicao"
    )
...
def test_erros_membros():
    ...
    data = [
        (
            pd.DataFrame(
                {"cod_mandato": [1, 1, 1], "membro_ativo": [True, False, True]}
            ),
        ...
        (
            pd.DataFrame(
                {"cod_mandato": [1, 1, 1], "membro_ativo": [False, False, False]}
            ),
        ...
        (
            pd.DataFrame(
                {"cod_mandato": [1, 1, 1], "membro_ativo": [True, True, True]}
            ),
            pd.Series([True, True, True], name="possui_membros"),
            pd.Series([None, None, None], name="erros_mandato_membros"),
        ),
